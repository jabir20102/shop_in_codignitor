	...
	public function create_tutorial($action = ''){
		if($this->session->userdata('teacher_login')){
			...
			// if create tutorial form is submitted
				...
				// calling model to save new tutorial information in database
				$result = $this->crud_model->add_tutorial($thumbnail, $preview);

				if($result){
					// if new tutorial is created
            		$this->session->set_flashdata('error', 'Tutorial Created Successfuly');
