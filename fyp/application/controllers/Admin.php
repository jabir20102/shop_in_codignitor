...
class Admin extends CI_Controller{
	...
	*/
	...
	public function teachers($action = ''){

		if($this->session->userdata('admin_login')){
			// if admin is logged in
			if($action == "delete"){
				// for deleting the selected teacher
				$teacher_id = $this->input->post('teacher-id');
				$result = $this->user_model->delete_teacher($teacher_id);
				$error = $this->db->error();
				// exit();
				if($result){
					// if teacher is successfuly deleted
					$this->session->set_flashdata('error', 'Student Deleted');
					$this->session->set_flashdata('class', 'alert alert-success mt-3');
					redirect(base_url('admin/teachers'),'refresh');
				}
				else{
					// if cannot delete the teacher
					$this->session->set_flashdata('error', $error['message']);
					$this->session->set_flashdata('class', 'alert alert-danger mt-3');
					redirect(base_url('admin/teachers'),'refresh');
				}
			...
			elseif($action == ''){
				// if delete action is not selected
				...
				// if any thing passed to action elso delete
				redirect(base_url('admin/teachers'),'refresh');
			}
	...
	public function all_tutorials($action = ''){
		...
		if($this->session->userdata('admin_login')){
			...
			elseif($action == 'delete'){
				...
				$result = $this->crud_model->delete_tutorial($tutorial_id);
	...
	public function reported_tutorials($action = ''){

		if($this->session->userdata('admin_login')){

			if($action == ''){
				$data['page_title'] = 'Reported Tutorials';
				$data['page_name'] = 'reported_tutorials';
				$data['active'] = 'reported-tutorials';
				$this->load->view('backend/index', $data, FALSE);
			}
			elseif($action == 'delete-report'){
				$report_id = $this->input->post('report-id');

				if(!isset($report_id)){
					// if link delete-report is directly accessed
					$this->session->set_flashdata('error', 'Select a Report to delete');
					$this->session->set_flashdata('class', 'alert alert-info mt-3');
					redirect(base_url('admin/reported-tutorials'),'refresh');
				}

				$result = $this->crud_model->delete_report($report_id);

				if($result){
					// if tutorial is successfuly deleted
					$this->session->set_flashdata('error', 'Report Deleted');
					$this->session->set_flashdata('class', 'alert alert-success mt-3');
					redirect(base_url('admin/reported-tutorials'),'refresh');
				}
				else{
